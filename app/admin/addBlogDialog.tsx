...
import { IconCameraPlus, IconList, IconPlus, IconTrash, IconFolderPlus } from "@tabler/icons-react";
...
function AddBlogDialog({
    ...
    const [sectionTitleBuffer, setSectionTitleBuffer] = useState("");
    ...
    function buildElement(element: any) {
        ...
        if (element.type.includes('heading')) {
            res = <div className={`text-[${getElementTS(element.type)}px] ${element.type.includes('heading') ? 'font-bold' : ''}`}>{element.value}</div>
    ...
    const prepareBlogData = () => {
        ...
        sections.forEach((section) => {
            section.elements.forEach((element) => {
                elements.push(new BlogElement(-1, element.type, element.value, -1, section.title || null, element.order, null, null));
            });
        });
                        ...
                        <div className="rounded-md p-4 border-[2px] border-gray-300 min-h-[100px] bg-white">
                            ...
                            {sections.map((section) => (
                                <div key={section.id} className="mb-6">
                                    ...
                                    {section.elements.map((element) => buildElement(element))}
                                </div>
                            ))}
                        </div>
                    </div>
            ...
            <DialogActions sx={{ paddingBottom: 3 }}>
                <Button
                    onClick={() => {
                        ...
                        addBlog(preparedBlog);
                    }}
                    sx={{ borderRadius: 10, paddingX: 6, backgroundColor: "orange" }}
                    variant="contained"
                    disabled={!blogData.title || sections.length === 0}
                >
                    Add
                </Button>
                <Button
